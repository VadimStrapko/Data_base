...
/* ex_1 */
set nocount on; --используется для отключения сообщений о количестве обработанных строк, которые возвращается при выполнении запроса.
...
begin catch
	...
	if @@TRANCOUNT > 0
		begin
			print 'Контольная точка: ' + @savepoint;
...
-- A ---
	...
	-------------------------- t1 --------------------
	select @@SPID 'SID' update AUDITORIUM set AUDITORIUM_CAPACITY = '15' where AUDITORIUM_NAME='301-1';	
...
--- B ---	
	...
	-- Транзакция A (начальная)
set transaction isolation level SERIALIZABLE;
begin transaction;
	delete FROM SUBJECT where SUBJECT = 'СТПИ';
	INSERT INTO SUBJECT VALUES ('СТПИ', 'Современные технологии программирования в интернете', 'ИСиТ');
	update SUBJECT SET SUBJECT_NAME = 'Современные технологии программирования в INTERNET' WHERE SUBJECT = 'СТПИ';
	select SUBJECT_NAME, PULPIT FROM SUBJECT WHERE PULPIT = 'ИСиТ';
-- Конец транзакции A
