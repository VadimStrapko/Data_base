...
go
print 'Кол-во студентов = ' + cast(dbo.COUNT_STUDENTS('ТОВ') as varchar(4))
...
go
print 'Кол-во студентов = ' + cast(dbo.COUNT_STUDENTS('ТОВ','1-48 01 02') as varchar(4))
print 'Кол-во студентов = ' + cast(dbo.COUNT_STUDENTS(default,'1-48 01 02') as varchar(4))
...
select 'ИСиТ',dbo.FCTEACHER('ИСиТ')[Количество преподавателей на ИСиТ]
...
as begin 
	declare @rc int = (select SUM(Количество) from РАСХОДЫ
	where РАСХОДЫ.Товар = ISNULL(@detailName, РАСХОДЫ.Товар));
	return @rc;
end;
go

select dbo.GetCountOfDetailsByName(null)[Всего Деталей]
select dbo.GetCountOfDetailsByName('Стол')[Всего деталей по имени детали]
select distinct РАСХОДЫ.Товар, dbo.GetCountOfDetailsByName(РАСХОДЫ.Товар)[Всего деталей по имени детали]
from РАСХОДЫ
